...
public class JobApplication extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		writer.append("<html>").append("<head>").append("</head>").append("<body>")
				...
				.append(name).append(email).append(phone).append(alternativePhoneNo).append(redio).append(qualification)
				.append(yop).append(university).append(address).append(skill).append(salary).append(exp).append(idproof)
				.append(idProofNumber)

				.append("</body>").append("</html>");
		JobApplicationEntity entity = new JobApplicationEntity(name, email, phone, alternativePhoneNo, redio,
				qualification, yop, university, address, skill, salary, exp, idproof, idProofNumber);
